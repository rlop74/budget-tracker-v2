...
export const RecentTransactions = () => {
  const transactions = useAccountDataStore((state) => state.allTransactions);
    ...
    <div className="mt-4 overflow-y-auto max-h-60">
      ...
      <div className="grid grid-cols-3 bg-violet-50 p-3 text-violet-800 font-semibold rounded-full sticky top-0">
        {tableHeaders.map((header, index) => (
          <header key={index} className="list-none text-md text-center">
            {header}
          </header>
        ))}
      </div>
          ...
          [...transactions]
            ...
            .map((transaction) => (
              ...
              >
                ...
                <p className="text-right capitalize">{transaction.name}</p>
              </li>
            ))
        )}
      </div>
