...
export const Dashboard = () => {
  ...
  const totalBalance = Number(user.income) - (totalExpense + totalSavings);

  // handle functions
  const handleAddExpense = async () => {
    // Validate inputs
    if (!newExpense.name.trim() || !newExpense.amount || !newExpense.category) {
    ...
    try {
      ...
      const data = await addExpense(expensePayload);
      if (!data) return;
      addNewExpense(data);
  ...
  const handleAddSavings = async () => {
    try {
      ...
      const data = await addSavings(savingsPayload);
  ...
  const handleUpdateIncome = async () => {
    ...
    try {
      await updateIncome(updatedIncome, user.auth_id);
