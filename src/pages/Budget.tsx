...
export const Budget = () => {
  ...
  const categorizedMonthlyExp: Record<string, number> = {};
  thisMonthExpenses.map((exp: Expense) => {
    ...
    categorizedMonthlyExp[exp.category] += Number(exp.amount);
  });
  ...
  const totalBudget = allBudgets.reduce((sum, c) => sum + Number(c.budget), 0);
  const totalSpent = allBudgets.reduce((sum, c) => sum + Number(c.spent), 0);
  ...
  return (
    <div className="flex flex-col gap-12 min-h-screen py-12 px-12">
      <div className="flex justify-between items-center">
        <h1 className="text-4xl font-extrabold">Monthly Budget Overview</h1>
      </div>
      ...
      <div className="flex flex-col gap-15">
        ...
        <div className="border border-gray-200 rounded-3xl shadow-2xl p-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className={`text-center py-4 px-8 rounded-2xl ${color}`}>
        ...
        <div className="grid grid-cols-2 gap-6">
          {[...allBudgets]
            ...
            .map((cat) => {
              const percentage = (Number(cat.spent) / Number(cat.budget)) * 100;
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-600 dark:text-slate-400">
                        ${Number(cat.spent).toLocaleString()} spent
                      </span>
                      <span className="text-slate-500 dark:text-slate-500">
                        ${Number(cat.budget).toLocaleString()} budget
                      </span>
                    </div>
                    ...
                    <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-3 overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-700 ${
                          isOverBudget
                            ? 'bg-linear-to-r from-red-500 to-red-800'
                            : 'bg-linear-to-r from-violet-500 to-violet-800'
                        }`}
                        style={{
                    ...
                    >
                      {isOver
                        ? `${Math.abs(
                            Number(cat.budget) - Number(cat.spent),
                          ).toLocaleString()} over`
                        : `${(Number(cat.budget) - Number(cat.spent)).toLocaleString()} left`}
                    </p>
                  </div>
                </div>
              );
